...
wss.on('connection', function connection(ws) {
    ...
    ws.on('message', function message(data) {
        ...
        console.log('sending %s to %s', JSON.stringify(res), JSON.stringify(JSON.parse(data).userId));
        ...
        if(data.action === "joinToGame"){
            ...
            clients.get(res.game.X).send(JSON.stringify({
                status:"success",
    ...
    ws.on('close', () => {
        ...
        if(game && otherPlayer !== 'bot'){
            clients.get(otherPlayer).send(JSON.stringify(processData({action:'playerDisconnected'})))
        }
        // Remove the client from the map
        clients.delete(userID);
        console.log(`${clients.size} connected.`)
    })
    console.log('sending: %s', JSON.stringify({userId: userID, status: "success", message: "Assigned userId", type: "userId"}));
    ws.send(JSON.stringify({userId: userID, status: "success", message: "Assigned user id", type: "userId"}))});
...
function processData(data){
    ...
    switch (data.action){
        ...
        case 'newGame':{
            ...
            games.set(gameId, {X: data.userId, O: null, board: new Array(9), next: "X", bot:false});
            result.type = 'newGame';
        ...
        case 'newBotGame':{
            const gameId = crypto.randomUUID();
            games.set(gameId, {X: data.userId, O: 'bot', board: new Array(9), next: "X", bot:true});
...
function makeBotMove(game) {
    ...
    const newBoard = game.board[openFields[getRandomInt(openFields.length - 1)]] = 'O';
